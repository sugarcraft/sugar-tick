...
| 8 | CandyShell | 🟡 | 30% | Bin script + Symfony Console application + 4 of 13 subcommands: `style` (non-interactive Sprinkles styling, hex/256/16 palette, padding/margin/width/align), `choose` (single-pick ItemList), `input` (single-line TextInput with password mode), `confirm` (y/n with exit codes 0/1/2 for yes/no/abort). Remaining: `spin`, `filter`, `format`, `pager`, `table`, `write`, `file`, `log`, `join`. |
