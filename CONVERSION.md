...
| 5 | SugarBits | 🟡 | 80% | Landed: `Key\Binding`/`Help`/`KeyMap`, `Help\Help`, `Spinner\Spinner` + 7 styles, `Progress\Progress`, `Timer\Timer` + `Stopwatch\Stopwatch`, `Cursor\Cursor`, `TextInput\TextInput`, **`TextArea\TextArea`** (multi-line, Enter splits, Backspace merges, multibyte-safe, char limit), **`Viewport\Viewport`** (line/half-page/page nav, gotoTop/Bottom, scrollPercent, vim-style keys), **`Paginator\Paginator`** (Dots/Arabic types, sliceBounds, vim/arrow/PgUp-Dn nav). Remaining: `List`, `Table`, `FilePicker`. |
