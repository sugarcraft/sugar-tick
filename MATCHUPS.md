...
| [charmbracelet/x/xpty](https://github.com/charmbracelet/x/tree/main/xpty) | **CandyPty** | `candy-pty/` | `sugarcraft/candy-pty` | `SugarCraft\Pty` | 🟢 | PTY primitive (Linux + macOS) — `Pty::open / spawn / read / write / resize / size / setBlocking`, `Child` lifecycle, `SignalForwarder` (SIGWINCH → resize) via FFI to libc; TIOCSCTTY deferred to candy-wish in-process upgrade |
