...
| [`x/xpty`](https://github.com/charmbracelet/x/tree/main/xpty) | Pseudo-terminal primitive — open / spawn / read / write / resize a PTY pair to drive child processes | 🟢 | Ported to `candy-pty/` — `Pty::open / spawn / read / write / setBlocking / resize / size / stream / close` (Linux + macOS via FFI to libc), readonly `Master` DTO, `Child` lifecycle (`pid / wait / exited / exitCode` plus `__destruct` zombie-reap safety net), `SizeIoctl` (TIOCSWINSZ + TIOCGWINSZ for both platforms), `SignalForwarder::attachSigwinch / attachSigchld` (SIGWINCH → `Pty::resize` via caller-supplied size provider), opt-in `controllingTerminal: true` on `spawn()` routing through `bin/pty-shim.php` for `setsid()` + `ioctl(TIOCSCTTY)` + `pcntl_exec` so Ctrl+C → SIGINT reaches the child (interactive shells / editors work cleanly). EINTR retry on `stream_select`, `SUGARCRAFT_LIBC` env override, examples (spawn-bash, pump-output, resize-forwarding), macOS CI matrix. All PRs merged (PR1-PR7). |
